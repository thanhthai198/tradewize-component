...
const FloatButton = ({
  ...
  (0, _react.useEffect)(() => {
    if (initialPosition) {
      ...
      const defaultPos = {
        x: (0, _utils.getScreenWidth)() - size - MIN_EDGE_DISTANCE,
        y: (0, _utils.getScreenHeight)() - size - (0, _utils.getScreenHeight)() * 0.25
      };
  ...
  (0, _react.useEffect)(() => {
    const {
      horizontal,
      vertical
    } = (0, _utils2.getDirection)(buttonPosition);
  ...
  const panResponder = (0, _react.useRef)(_reactNative.PanResponder.create({
    ...
    onPanResponderRelease: () => {
      ...
      if (newX < (0, _utils.getScreenWidth)() / 2) {
        finalX = minEdgeDistance;
      } else {
        finalX = (0, _utils.getScreenWidth)() - size - minEdgeDistance;
      }

      // Ensure button stays within screen bounds
      if (finalY < yAxisStartLimit) {
        finalY = yAxisStartLimit;
      } else if (finalY > (0, _utils.getScreenHeight)() - size - yAxisStartLimit - yAxisEndLimit) {
        finalY = (0, _utils.getScreenHeight)() - size - yAxisStartLimit - yAxisEndLimit;
      }
      const finalPosition = {
        x: finalX,
        y: finalY
