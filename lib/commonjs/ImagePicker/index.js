...
const ImagePickerComponent = ({
    ...
    try {
      if (_reactNative.Platform.OS === 'android') {
        ...
        if (_reactNative.Platform.Version >= 33) {
          const result = await (0, _reactNativePermissions.request)(_reactNativePermissions.PERMISSIONS.ANDROID.READ_MEDIA_IMAGES);
          return result === _reactNativePermissions.RESULTS.GRANTED;
        } else {
  ...
  const openCamera = (0, _react.useCallback)(async () => {
    try {
      ...
      if (!hasPermission) {
        _reactNative.Alert.alert('Permission Required', 'Camera permission is required to take photos. Please enable it in settings.', [{
          text: 'OK'
        }]);
        return;
      }
    ...
    if (multiple) {
      ...
      if (newSelected.has(index)) {
        ...
        if (newSelected.size >= maxFiles) {
          _reactNative.Alert.alert('Limit Reached', `You can only select up to ${maxFiles} images.`, [{
            text: 'OK'
          }]);
          return;
        }
  ...
  return /*#__PURE__*/_react.default.createElement(_reactNative.View, {
  ...
  }, buttonText)), renderSelectedImages(), /*#__PURE__*/_react.default.createElement(_reactNative.Modal, {
  ...
  }, multiple ? `Select Photos (${selectedLimitedPhotos.size}/${maxFiles})` : 'Select Photo'), /*#__PURE__*/_react.default.createElement(_reactNative.TouchableOpacity, {
    ...
    style: [styles.confirmButton, selectedLimitedPhotos.size === 0 && styles.disabledConfirmButton],
