...
exports.default = exports.ImagePickerComponent = void 0;
...
const ImagePickerComponent = ({
  ...
  const validateImage = (0, _react.useCallback)(image => {
  ...
  const processImage = (0, _react.useCallback)(image => {
  ...
  const openCamera = (0, _react.useCallback)(async () => {
    try {
      ...
      const image = await _reactNativeImageCropPicker.default.openCamera(options);
  ...
  const openGallery = (0, _react.useCallback)(async () => {
    try {
      ...
      const images = await _reactNativeImageCropPicker.default.openPicker(options);
  ...
  const showImagePicker = (0, _react.useCallback)(() => {
    if (showActionSheet) {
      ...
      _reactNative.Alert.alert(actionSheetOptions.title || 'Chọn ảnh từ', '', [{
